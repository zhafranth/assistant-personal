...
func (r *Repository) Create(ctx context.Context, userID int64, description string, amount int64) (int, error) {
	var id int
	err := r.db.QueryRowContext(ctx,
		`INSERT INTO expenses (user_id, description, amount) VALUES ($1, $2, $3) RETURNING id`,
...
func (r *Repository) List(ctx context.Context, userID int64, filter string, loc *time.Location) ([]Expense, error) {
	...
	case "today":
		...
		query = `SELECT id, user_id, description, amount, recorded_at FROM expenses
	...
	for rows.Next() {
		var e Expense
		if err := rows.Scan(&e.ID, &e.UserID, &e.Description, &e.Amount, &e.RecordedAt); err != nil {
...
func (r *Repository) FindBySearch(ctx context.Context, userID int64, search string) (*Expense, error) {
	var e Expense
	err := r.db.QueryRowContext(ctx,
		`SELECT id, user_id, description, amount, recorded_at FROM expenses
		 WHERE user_id = $1 AND description ILIKE '%' || $2 || '%'
		 ORDER BY recorded_at DESC LIMIT 1`,
		userID, search,
	).Scan(&e.ID, &e.UserID, &e.Description, &e.Amount, &e.RecordedAt)
